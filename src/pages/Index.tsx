...
const Index = () => {
    ...
    const fetchPrompts = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from("prompts")
          .select("id, title, prompt, category, created_at");
        ...
        if (!data || data.length === 0) {
          ...
          toast({
            title: "אין פרומפטים",
            description: "לא נמצאו פרומפטים בטבלה",
          });
          setPrompts([]);
          setCategories([]);
          setLoading(false);
        ...
        console.log("Loaded", formatted.length, "prompts");
        setLoading(false);
  ...
  // Load favorites, theme, and PayPal SDK
  useEffect(() => {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div id="paypal-container-TWSW6SFMDNR72" className="mr-4"></div>
              <Button
                variant="outline"
                size="icon"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPrompts.map((prompt) => (
              ...
              >
                ...
                <h3 className="text-lg font-semibold mb-3 text-foreground">{prompt.title}</h3>
                <p className="text-sm text-muted-foreground mb-4 flex-grow leading-relaxed">{prompt.content}</p>
                <Button
